...
class PointcloudScaleAndTranslate(object):
    ...
    def __call__(self, pc):
        ...
        pc[:, 0:3] = (
            torch.mul(pc[:, 0:3], torch.from_numpy(xyz1).float())
...
class PointcloudScaleAndJitter(object):
    ...
    def __call__(self, pc):
        ...
        symmetries = symmetries * np.array(self.augment_symmetries) + (
            1 - np.array(self.augment_symmetries)
        )
        xyz1 *= symmetries
        xyz2 = np.clip(
            np.random.normal(scale=self.std, size=[pc.shape[0], 3]),
            a_min=-self.clip,
            a_max=self.clip,
        )
        pc[:, 0:3] = (
...
class BatchPointcloudScaleAndJitter(object):
    ...
    def __call__(self, pc):
        ...
        for i in range(bsize):
            ...
            xyz1 *= symmetries
            xyz2 = np.clip(
                np.random.normal(scale=self.std, size=[npoint, 3]),
                a_max=self.clip,
                a_min=-self.clip,
            )
