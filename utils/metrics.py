...
def calculate_contrast(points_labels_flats, pred_soft_flats):
    """
    Function calculates contrast metric on point cloud
...
def compute_metrics_on_voxels(y_pred, y_true):
    ...
    assert y_pred.shape == y_true.shape, "Predictions and ground-truth have different sizes"
...
def get_top_10_metric_and_mask(config, subject, prediction, label, crop_size=64):
    ...
    df = pd.DataFrame(columns = ['mean_prediction',
                                 'mean_label',
                                 'sagittal_iter',
                                 'coronal_iter', 
                                 'axial_iter'])
        ...
        for coronal_iter in range(math.ceil(coronal_shape / crop_size)):
            for axial_iter in range(math.ceil(axial_shape / crop_size)):
                ...
                prediction_crop = prediction[
                    sagittal_iter * crop_size : min((sagittal_iter + 1) * crop_size, sagittal_shape),
                    coronal_iter * crop_size : min((coronal_iter + 1) * crop_size, coronal_shape),
                    axial_iter * crop_size : min((axial_iter + 1) * crop_size, axial_shape),
                ]
    ...
    df = df.sort_values(['mean_prediction','mean_label'], ascending = False).reset_index(drop = True)
    ...
    for i in top_10_df.index:
        sagittal_iter, coronal_iter, axial_iter = top_10_df.loc[i, ['sagittal_iter', 'coronal_iter', 'axial_iter']].astype(int)
    ...
    nib.save(mask, f"experiments/{config.EXP_NAME}/top_10_masks/{subject}.nii")
