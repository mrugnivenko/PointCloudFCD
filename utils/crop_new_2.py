...
def sample_without_repetition(population, sampleSize):
    """"""
    random.seed(39)
...
def get_random_crop(
    ...
    if task == "val":
        np.random.seed(int(single_data_dict["labels"].sum()))
        random.seed(int(single_data_dict["labels"].sum()))
    ...
    if coin_flip:
        ...
        while not is_break:
            ...
            if (brain_crop.sum() != 0) and mask_crop.sum() == 0:
                ...
                is_break = False
                k += 1
                np.random.seed(int(single_data_dict["labels"].sum()) + k)
                random.seed(int(single_data_dict["labels"].sum()) + k)
        ...
        while not is_break:
            ...
            if (
                (brain_crop.sum() != 0)
                and (brain_crop.shape == (crop_size, crop_size, crop_size))
                and ((brain_crop > 0).sum() >= 0)
            ):
                is_break = True
            else:
                is_break = False
...
class BrainDataSegCrop:
    ...
    def __init__(
        ...
        self.return_abs_coords = config.is_return_absolute_coordinates
        self.return_pc_without_air_points = config.get_rid_of_air_points
        self.coin_flip_threshold = config.coin_flip_threshold
        self.MEANS = MEANS
        self.STDS = STDS
        ...
        if task in ["train", "val"]:
            ...
            data_dict_loaded["labels"] = [
                (data_dict_loaded["labels"][0] > 0).astype(np.int8)
            ]
    ...
    def __getitem__(self, idx):
        ...
        if self.task in ["train", "val"]:
            ...
            for key in single_data_dict:
                single_data_dict[key] = load_nii_to_array(single_data_dict[key])
            single_data_dict["labels"] = (single_data_dict["labels"] > 0).astype(
                np.int8
            )
        ...
        if cur_num_points >= self.num_points:
            ...
            if self.task == "val":
                choice = sample_without_repetition(
                    range(cur_num_points), self.num_points
                )
            ...
            if self.task == "val":
                padding_choice = sample_without_repetition(
                    range(cur_num_points), padding_num
                )
        ...
        if self.transforms is not None and self.task == "train":
...
def dataset_to_best_weights(dataset, features: list, repeats: int = 2):
    ...
    for _ in range(repeats):
        for element in dataset:
            ...
            for key in features:
                dict_means[key].append(
                    torch.mean(current_points[:, dict_features_to_numbers[key]])
                    .detach()
                    .cpu()
                )
                dict_stds[key].append(
                    torch.std(current_points[:, dict_features_to_numbers[key]])
                    .detach()
                    .cpu()
                )
...
def get_loader_crop(
    ...
    if weighted_loss:
        weights, MEANS, STDS = dataset_to_best_weights(
            train_dataset, [x for x in train_dict.keys() if "labels" not in x]
        )
        ...
        for key in MEANS:
            config.MEANS[key] += [MEANS[key]]
            config.STDS[key] += [STDS[key]]
        #         config.MEANS =  MEANS
        #         config.STDS = STDS
